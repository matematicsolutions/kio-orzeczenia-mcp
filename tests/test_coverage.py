...
from kio_orzeczenia_mcp.coverage import build_coverage
from kio_orzeczenia_mcp.server import kio_coverage


def _coverage():
    fn = getattr(kio_coverage, "fn", kio_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KIO_MCP_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("kio_coverage" in line for line in lines), (
        "an audit entry was written but it does not name kio_coverage"
    )
